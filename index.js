...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  ...
  const realAge = monthDiff < 0 || (monthDiff === 0 && today.getDate() < dob.getDate()) ? age - 1 : age;
  if (realAge < 13) {
...
app.patch('/api/auth/me', requireAuth, async (req, res) => {
  ...
  const updates = {};
  if (username) {
    if (username.length < 3 || username.length > 30) return res.status(400).json({ error: 'Username inválido' });
    if (!/^[a-zA-Z0-9_.]+$/.test(username)) return res.status(400).json({ error: 'Username con caracteres inválidos' });
    const { data: existing } = await supabase.from('profiles').select('id').eq('username', username).neq('id', req.userId).single();
  ...
  const { data, error } = await supabase.from('profiles').update(updates).eq('id', req.userId).select('username, birth_date, avatar_url').single();
